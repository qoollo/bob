...
impl Node {
    ...
    pub async fn check(&self, client_factory: &Factory, local_node_name: &String) -> Result<(), String> {
        if let Some(conn) = self.get_connection().await {
            self.ping(&conn).await
        } else {
            debug!("will connect to {:?}", self);
            let client = client_factory.produce(self.clone(), local_node_name.clone()).await?;
