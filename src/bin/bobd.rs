use bob::api::grpc::{server, PutRequest, GetRequest, OpStatus, BlobResult, Null, BobError};
...
impl server::BobApi for BobSrv {
        ...
        fn put(&mut self, req: Request<PutRequest>) -> Self::PutFuture {
            ...
            if !self.put_validate(&param) {
                ...
                Box::new(future::err(tower_grpc::Status::new(tower_grpc::Code::InvalidArgument, "Key and data is mandatory")))
            } else {
                let key = BobKey{
                ...
                Box::new(self.grinder.put(key,data,
                    ...
                    match r {
                        Ok(r_ok) => {
                            info!("PUT[{}]-OK local:{} ok dt: {}ms", key, !r_ok.is_clustered, elapsed);
                            ok(Response::new(OpStatus {
                                error: None
                            }))
                        },
                        Err(r_err) => {
                            error!("PUT[{}]-ERR local:{}  dt: {}ms {}", key, !r_err.is_clustered, elapsed, r_err.to_string());
                            ok(Response::new(OpStatus {
                                error: Some(BobError {
                                    code: 1,
...
fn main() {
    env_logger::builder()
            .filter_module("bob", LevelFilter::Info).init();
